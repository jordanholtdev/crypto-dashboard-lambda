...
async function getCoins() {
    ...
    const allCoins = await knex
        ...
        .limit(50);
    const allPrices = await knex
        .select('price_usd', 'coin_id', 'created_date')
        .from('coin_price')
        .limit(48)
...
async function getPortfolio() {
    ...
    const activity = await knex
        .select('*')
        .from('holdings')
        .orderBy([{ column: 'purchase_date', order: 'desc' }]);
    ...
    const currentPrice = await knex
        .select('*')
        .from('coin_price')
        .orderBy([{ column: 'created_date', order: 'desc' }])
        .limit(10);
